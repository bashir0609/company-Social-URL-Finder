...
- **90%+ Success Rate**: Enhanced algorithms for maximum accuracy
...
- **Contact Page Scraping**: Finds additional links on contact pages
- **80+ Website Patterns**: Supports 20+ TLDs and multiple name variations
...
### Success Rate: 90%+
Our enhanced algorithms achieve **90%+ success rate** through:
- **80+ website patterns** across 20+ TLDs (com, io, net, org, co, ai, dev, etc.)
- **5 name variations** per social platform (no spaces, hyphenated, underscored, etc.)
- **Smart retry logic** with exponential backoff (3 attempts per request)
- **Contact page scraping** for additional social links
- **Fallback strategies** (HEAD → GET requests)
- **Extended timeouts** (25s for websites, 8s for discovery)

### Performance Metrics
- **Cold Start**: ~1-2 seconds (Vercel serverless)
- **Single Search**: ~5-10 seconds per company (comprehensive)
- **Bulk Processing**: Sequential (1 company at a time)
- **Max Request Time**: 10 seconds (Vercel limit)
- **Average Social Profiles Found**: 3-5 per company

### Optimization Tips
- Use bulk processing for multiple companies
- Results are processed client-side for better UX
- Serverless functions scale automatically
- See `IMPROVEMENTS.md` for detailed technical breakdown
